...
// Plane Y velocity — persists across frames for smooth S-curve easing
var planeVelY = 0;
...
function resetGame(){
  ...
  game = {speed:0,
          ...
          planeAmpHeight:80,
          planeAmpWidth:75,
          planeMoveSensivity:0.02,
          planeRotXSensivity:0.0032,
          planeRotZSensivity:0.0016,
          planeFallSpeed:.001,
          planeMinSpeed:1.2,
          planeMaxSpeed:1.6,
          planeSpeed:0,
...
function createScene() {
  ...
  // Optimization: Turned off antialiasing for maximum frame rate
  renderer = new THREE.WebGLRenderer({ alpha: true, antialias: false });
  renderer.setSize(WIDTH, HEIGHT);

  renderer.shadowMap.enabled = false; // Shadows OFF for maximum GPU performance
...
function createLights() {
  ...
  // Optimization: Lowered shadow map size from 4096 to 512 to prevent GPU stalling
...
Sky.prototype.moveClouds = function(){
  // Removed per-block random rotation from moveClouds to eliminate hundreds of Math.random() calls per frame
  this.mesh.rotation.z += game.speed*deltaTime;
}

Sea = function(){
  // Optimization: Reduced cylinder segments from 40 to 20 — halves vertex count for sea waves
  var geom = new THREE.CylinderGeometry(game.seaRadius,game.seaRadius,game.seaLength,20,10);
...
Sea.prototype.moveWaves = function (){
  ...
  // Optimization: Flag for update ONCE per frame, not per vertex!
...
function updateDiamonds(){
  // Track distance internally (still used for spawning), but show diamonds to user
  game.distance += game.speed*deltaTime*game.ratioSpeedDistance;
  fieldDiamonds.innerHTML = game.diamondsCollected;
  // Level ring still advances by distance internally
...
function removeEnergy(){
  // Instant game-over on enemy hit (one hit = dead)
  game.status = "gameover";
}
...
function updatePlane(){
  // Constant forward speed — X axis removed from hand control
  game.planeSpeed = 1.4;

  var targetY = normalize(mousePos.y, -.75, .75,
    game.planeDefaultHeight - game.planeAmpHeight,
    game.planeDefaultHeight + game.planeAmpHeight)
    + game.planeCollisionDisplacementY;

  var dy = targetY - airplane.mesh.position.y;

  // S-curve velocity spring:
  // desiredVel shrinks as plane nears target → natural ease-out
  // velocity lerps toward desiredVel → smooth ease-in from 0
  var vt = Math.min(1.0, deltaTime * 0.032);
  planeVelY += (dy * 0.25 - planeVelY) * vt;
  airplane.mesh.position.y += planeVelY;

  // Keep X at a fixed comfortable position
  airplane.mesh.position.x += (-game.planeAmpWidth * 0.85 - airplane.mesh.position.x) * Math.min(1.0, deltaTime * 0.015);

  // Bank angle = current velocity magnitude, not position gap
  // → tilts INTO the climb/descent; levels out as speed decelerates on arrival
  var rt = Math.min(1.0, deltaTime * 0.025);
  airplane.mesh.rotation.z += ((planeVelY / game.planeAmpHeight) * 5.5 - airplane.mesh.rotation.z) * rt;
  airplane.mesh.rotation.x += (-(planeVelY / game.planeAmpHeight) * 2.8 - airplane.mesh.rotation.x) * rt;

  camera.position.y += (airplane.mesh.position.y - camera.position.y) * Math.min(1.0, deltaTime * 0.012);
...
var fieldDiamonds, energyBar, replayMessage, fieldLevel, levelCircle;

function init(event){

  // UI
  fieldDiamonds = document.getElementById("distValue");
